...
-(void)viewWillAppear:(BOOL)animated{
    ...
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSError *error;
        NSString *link=@"https://www.softrockgroup.com/droid-product/";
        link=[link stringByAppendingString:self->_id];
        NSData *data1 = [NSData dataWithContentsOfURL:[NSURL URLWithString:link]];
        ...
        {
            self->currentArray=json1;
            dispatch_async(dispatch_get_main_queue(), ^{
                [_collectionViewForAlldata reloadData];
